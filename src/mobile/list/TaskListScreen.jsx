...
const SCOPE_TABS = [
  { key: "gorevler", label: "Planlı Bakım" },
  { key: "kiracitalepleri", label: "Kiracı" },
];
...
export function TaskListScreen({ state, updateState, currentUserName, scope, pendingAction, onConsumePending, canWrite = true }) {
  ...
  function saveTask() {
    ...
    const payload = { ...form, id, createdAt: form.createdAt || new Date().toISOString(), createdBy: form.createdBy || currentUserName, updatedAt: new Date().toISOString(), updatedBy: currentUserName };
  ...
  // forma yönlendirilir (bkz. DetailScreen actions).
  function advanceStatus(task, status) {
    const payload = { ...task, status, updatedAt: new Date().toISOString(), updatedBy: currentUserName };
